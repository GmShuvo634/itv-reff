    ...
    <div className="min-h-screen bg-gray-50">
          ...
          <div className="flex justify-between items-center py-4">
            <div className="flex items-center">
              <div className="relative w-8 h-8">
                <img
                  src="/logo.svg"
                  alt="Position Level System"
                  className="w-full h-full object-contain"
                />
              </div>
              <span className="ml-2 text-lg font-semibold">Position Level System</span>
            </div>
            <nav className="flex space-x-4">
              <a href="/dashboard" className="text-gray-600 hover:text-gray-900">Dashboard</a>
              <a href="/positions" className="text-blue-600 font-medium">Positions</a>
            </nav>
          </div>
        </div>
      </header>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
          ...
          <Card className="mb-8 border-green-200 bg-green-50">
            <CardHeader>
              ...
              <CardDescription className="text-green-700">
                {currentPosition.isExpired ? 'Position has expired' : `${currentPosition.daysRemaining} days remaining`}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div>
                  ...
                  <p className="font-semibold text-green-800">
                    {currentPosition.tasksCompletedToday}/{currentPosition.currentPosition.tasksPerDay}
                  </p>
                </div>
                <div>
                ...
                <div>
                  ...
                  <p className="font-semibold text-green-800">
                    {formatCurrency(currentPosition.currentPosition.dailyIncome)}
                  </p>
                </div>
                <div>
        ...
        <div className="text-center mb-12">
          <h1 className="text-4xl font-bold text-gray-900 mb-4">Position Level System</h1>
          <p className="text-xl text-gray-600 max-w-3xl mx-auto">
            Choose your position level to start earning. Higher levels require deposits but offer better rewards and income potential.
          </p>
        </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mb-12">
          {positions.map((position) => {
            const isCurrentPosition = currentPosition?.currentPosition?.id === position.id;
            ...
            return (
              ...
              >
                ...
                <CardHeader className="text-center pb-4">
                  ...
                  <CardDescription>
                    {isIntern ? 'Entry Level - No Deposit Required' : `Level ${position.level} Position`}
                  </CardDescription>
                  <div className="mt-4">
                ...
                <CardContent className="space-y-4">
                  <div className="space-y-3">
                    <div className="flex items-center gap-3">
                      ...
                      <span className="text-sm">{position.tasksPerDay} tasks daily</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Gift className="h-4 w-4 text-green-600" />
                      <span className="text-sm">{formatCurrency(position.unitPrice)} per task</span>
                    </div>
                    <div className="flex items-center gap-3">
                      <Clock className="h-4 w-4 text-orange-600" />
                      <span className="text-sm">{position.validityDays} days validity</span>
                    </div>
                  </div>
